...
# Package data model containing type annotations.
REQUIRED_COLUMN = "Image"
...
class CaptureTableModel(QAbstractTableModel):
    def __init__(self, frame: pl.DataFrame | None = None, parent: Any = None) -> None:
        ...
        self._frame: pl.DataFrame = self._normalize_frame(
            frame or _empty_frame([REQUIRED_COLUMN])
        )
    ...
    def _replace_frame(self, frame: pl.DataFrame) -> None:
        ...
        self._frame: pl.DataFrame = self._normalize_frame(frame)
        self.endResetModel()
    ...
    def setData(
        ...
        rows: list[dict[str, Any]] = self._frame.to_dicts()
    ...
    def remove_column(self, column_name: str) -> None:
        ...
        columns: list[str] = [
            column for column in self._frame.columns if column != column_name
        ]
        self._replace_frame(_frame_from_rows(rows, columns))
