...
class CameraService(QObject):
    ...
    def start_preview(self, mode: CameraMode | None = None) -> None:
        ...
        device_index = self._preview_device_index(self._mode)
    ...
    def capture_still(
        ...
        capture_callable = self._build_capture_callable(destination, metadata or {})
    ...
    def _build_capture_callable(
        ...
        def capture() -> CaptureResult:
            if mode == CameraMode.DSLR_WIA:
                source = WiaCaptureSource(self._config.wia_device_name)
                image_path = source.capture(destination)
            else:
                source = WebcamCaptureSource(self._config.webcam_preview_device_index)
                image_path = source.capture(destination, self.latest_preview_frame())
