...
class WiaCaptureSource:
    ...
    def capture(self, destination: Path) -> Path:
        ...
        try:
            from win32com.client import (  # pylint: disable=import-outside-toplevel
                Dispatch,
            )
        ...
        try:
            device: Any = self._connect_device(Dispatch)
            item: Any = self._first_item(device)
            transfer: Any = self._transfer_item(Dispatch, item)
    ...
    def _connect_device(self, dispatch: Any) -> Any:
        manager: Any = dispatch("WIA.DeviceManager")
        device_infos: Any = getattr(manager, "DeviceInfos", None)
        if device_infos is None:
            raise CameraUnavailableError(
                "WIA device manager did not expose any devices"
            )

        count: int = int(getattr(device_infos, "Count", 0))
        if count < 1:
            raise CameraUnavailableError("No WIA devices were found")

        selected: Any = None
        for index in range(1, count + 1):
            info: Any = device_infos.Item(index)
    ...
    def _matches_device(self, device_info: Any) -> bool:
        ...
        try:
            properties: Any = device_info.Properties
            name: str = str(properties("Name").Value)
    ...
    def _first_item(self, device: Any) -> Any:
        items: Any = getattr(device, "Items", None)
        if items is None:
            raise CaptureFailedError("WIA device did not expose any capture items")

        count: int = int(getattr(items, "Count", 0))
        if count < 1:
    ...
    def _transfer_item(self, dispatch: Any, item: Any) -> Any:
        dialog: Any = dispatch("WIA.CommonDialog")
        transfer: Any = getattr(dialog, "ShowTransfer", None)
        if transfer is None:
